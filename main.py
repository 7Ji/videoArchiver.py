if __name__ != '__main__':
    raise RuntimeError('REFUSED to be importd, you should only run this as sciprt!')
...
def timedelta(t):
    """Convert str HH:MM:SS.MS to timedelta

    used in ffmpeg_time_size_poller (scope: child + main)
        used in encoder (scope: child/encoder)
        used in get_duration_and_size (wrapper)
            used in stream_info (scope: main)
            used in delta_adder (scope: child/encoder)
...
def ffmpeg_time_size_poller(
        p: subprocess.Popen,
        s_type: str,
        size_allow: int = None,
        progress_bar: ProgressBar = None,
        target_time: datetime.timedelta = None
):
    ...
    while p.poll() is None:
        ...
        if chars:
            ...
            if size_allow is not None:
                ...
                if m:
                    size_produced = int(m[1])
                    if size_produced >= size_allow or (
                            check_time and t > time_ten_seconds and size_produced/size_allow >= percent):
                        inefficient = True
                        p.kill()
                        break
                pass
            ...
            p.kill()
            break
    m = reg_complete[s_type].search(line)
    if m:
        s = int(m[1])
...
def get_duration_and_size(media: pathlib.Path, s_id: int, s_type: str):
    ...
    """
    p = subprocess.Popen(
        ('ffmpeg', '-i', media, '-c', 'copy', '-map', f'0:{s_id}', '-f', 'null', '-'),
        stdout=subprocess.DEVNULL, stderr=subprocess.PIPE)
    r, t, s = ffmpeg_time_size_poller(p, s_type)
    return t, s


def stream_info(file_raw, s_id, s_type, s):
    """Get stream info,
    ...
    scope: main
        End when KeyboardException is captured
    """
    duration, size = get_duration_and_size(file_raw, s_id, s_type)
    if s_type == 'video':
        lossless = s['codec_name'] in codec_vlo
        if 'side_data_list' in s and 'rotation' in s['side_data_list'][0]:
            return {
                'type': s_type,
                'lossless': lossless,
                'width': s['width'],
                ...
                'duration': duration,
                'size': size,
                'rotation': s['side_data_list'][0]['rotation']
            }
        return {
            'type': s_type,
            'lossless': lossless,
...
def stream_copy(d, prefix, file_raw, s_id, file_out, prompt_title, file_done):
    ...
    file_copy = d / f'{prefix}_copy.nut'
    args = ('ffmpeg', '-i', file_raw, '-c', 'copy', '-map', f'0:{s_id}', '-y', file_copy)
...
def concat(prefix, concat_list, file_out, prompt_title, file_done):
    """Concatenating multiple parts of a media file
    ...
    """
    CleanPrinter.print(f'{prompt_title} Transcode done, concatenating all parts')
    ...
    while ffmpeg_dumb_poller(subprocess.Popen(args, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)):
        check_end()
        CleanPrinter.print(f'{prompt_title} Concatenating failed, trying that later')
        time.sleep(5)
    shutil.move(file_concat, file_out)
    CleanPrinter.print(f'{prompt_title} Concatenating done')
    file_done.touch()


def args_constructor(start, file_raw, file_out, s_id, s_type, encode_type, stream_width, stream_height):
    """Constructing ffmpeg args for encoder
...
def scan_dir(d: pathlib.Path):
    ...
    for i in d.iterdir():
        if i.is_dir():
            ...
            if i not in db:
                r = subprocess.run(
                    ('ffprobe', '-show_format', '-show_streams', '-select_streams', 'V', '-of', 'json', i),
                ...
                if r.returncode == 0:
                    ...
                    if not (j_format.endswith('_pipe') or j_format in ('image2', 'tty')) and j['streams']:
                        ...
                        for s_id, s in enumerate(
                            json.loads(subprocess.run(('ffprobe', '-show_streams', '-of', 'json', i),
...
def delta_adder(file_check, s_type, start, size_exist):
    ...
    """
    time_delta, size_delta = get_duration_and_size(file_check, 0, s_type)
...
def encoder(
    ...
    encode_type: str,
    s_id: int, s_type: str, duration: datetime.timedelta, size_raw: int, lossless: bool,
    ...
    """
    try:
        if encode_type == 'preview' and s_type == 'audio':
            prompt_title = f'[{file_raw.name}] E:P S:A:{s_id}'
        else:
            prompt_title = f'[{file_raw.name}] E:{encode_type[:1].capitalize()} S:{s_id}:{s_type[:1].capitalize()}'
        CleanPrinter.print(f'{prompt_title} Work started')
        if encode_type == 'preview' and s_type == 'audio':
            prefix = f'{file_raw.stem}_preview_audio'
        else:
            prefix = f'{file_raw.stem}_{encode_type}_{s_id}_{s_type}'
        file_concat_pickle = d / f'{prefix}_concat.pkl'
        start = time_zero
        size_exist = 0
        concat_list = []
        ...
        if file_out.exists() and file_out.stat().st_size:
            ...
            if s_type == 'video':
                ...
                if ffmpeg_dumb_poller(p) == 0:
                    ...
                    if frame_last:
                        CleanPrinter.print(
                            f'{prompt_title} {frame_last} frames seem usable, trying to recovering those')
                        p = subprocess.Popen(('ffmpeg', '-i', file_out, '-c', 'copy', '-map', '0', '-y', '-vframes',
                                              str(frame_last), file_recovery), stdout=subprocess.DEVNULL,
                ...
                poll = True
                p = subprocess.Popen(('ffmpeg', '-i', file_out, '-c', 'copy', '-map', '0', '-y', file_recovery),
                                     stdout=subprocess.DEVNULL, stderr=subprocess.PIPE)
            if poll:
                r, t, s = ffmpeg_time_size_poller(p, s_type)
                if r:
        ...
        if concat_list:
            # We've already encoded this
            if check_efficiency and size_raw and size_exist > size_raw * 0.9:
                stream_copy(d, prefix, file_raw, s_id, file_out, prompt_title, file_done)
                return
            if start >= duration or duration - start < time_second:
                CleanPrinter.print(f'{prompt_title} Seems already finished, concatenating all failed parts')
        ...
        args = args_constructor(start, file_raw, file_out, s_id, s_type, encode_type, stream_width, stream_height)
        ...
        while True:
            ...
            if check_efficiency:
                if ffmpeg_time_size_poller(p, s_type, size_allow, progress_bar,
                                           target_time):  # return ture for inefficient
                    stream_copy(d, prefix, file_raw, s_id, file_out, prompt_title, file_done)
                    return
            else:
                ffmpeg_time_size_poller(p, s_type, progress_bar=progress_bar, target_time=target_time)
            ...
            if p.wait() == 0:
                ...
                break
            CleanPrinter.print(f'{prompt_title} Transcode failed, return code: {p.wait()}, retrying that later')
            time.sleep(5)
    except EndExecution:
        return
...
def muxer(
    ...
    muxer itself (scope: child/muxer)
...
def screenshooter(
    ...
    try:
        ...
        if length == 1:
            ...
            args = (*args_ffmpeg, '-ss', str(duration/2), '-i', file_raw, '-map', f'0:{s_id}', *args_out)
            prompt = f'{prompt_title} Taking screenshot, single frame'
        else:
            prompt_title = f'[{file_raw.name}] S:{s_id}'
            ...
            if duration < datetime.timedelta(seconds=tiles*5):
                args = (
                    *args_ffmpeg, '-i', file_raw, '-map', f'0:{s_id}', '-filter:v',
                    f'select=eq(n\,0)+gte(t-prev_selected_t\,{time_delta.total_seconds()}),tile={length}x{length}{arg_scale}', *args_out
                )
            else:
                ...
                for i in range(length):
                    for j in range(length):
                        ...
                        args_mapper.append(f'[{file_id}:{s_id}]')
                ...
                args = (
                    *args_ffmpeg, *args_input, '-filter_complex',
                    f'{arg_mapper}xstack=inputs={tiles}:layout={arg_position}{arg_scale}', *args_out
                )
        ...
        while True:
            ...
            if ffmpeg_dumb_poller(subprocess.Popen(args, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)):
                CleanPrinter.print(f'{prompt_title} Failed to screenshot, retrying that later')
            else:
                break
            time.sleep(5)
    except EndExecution:
...
def cleaner(
    ...
    try:
        for thread in threads:
            join(thread)
        CleanPrinter.print(f'[{file_raw.name}] Cleaning input and {d}...')
        shutil.rmtree(d)
        file_raw.unlink()
        check_end()
        with lock_db:
...
def thread_adder(d, file_raw, encode_type, s_id, s_type, s_duration, s_size, s_lossless, s_width, s_height, streams,
                 threads, amix=False):
    ...
    """
    if amix:
        file_stream = d / f'{file_raw.stem}_preview_audio.nut'
        file_s_done = d / f'{file_raw.stem}_preview_audio.done'
    else:
        file_stream = d / f'{file_raw.stem}_{encode_type}_{s_id}_{s_type}.nut'
        file_s_done = d / f'{file_raw.stem}_{encode_type}_{s_id}_{s_type}.done'
    streams.append(file_stream)
    if not file_stream.exists() and file_s_done.exists():
        file_s_done.unlink()
    if not file_s_done.exists():
        threads.append(threading.Thread(target=encoder, args=(
            d,
            i, file_stream, file_s_done,
            encode_type,
            s_id, s_type, s_duration, s_size, s_lossless,
            s_width, s_height
        )))
        threads[-1].start()
...
def db_cleaner(db_old):
    ...
    """
    db_new = {}
    for i_r, j_r in {i:j for i, j in db_old.items() if i.exists()}.items():
        name = f'{i_r.stem}.mkv'
        finish = False
